...
public class MainActivity extends BrowserHandler {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//THIS IS TEMPORARY CODE TO TRANSFER FROM THE OLD BOOKMARKS SYSTEM TO THE NEW ONE
  		...
  		for (int book = 0; book<numBookmarksToTransfer; book++){
  			...
  			mPrefs.edit().remove("bookmarktitle"+book).remove("bookmark"+book).apply();
  			Log.d("LB", "bookmark" + book +" "+url+ " transfered");
  		}
  		if (numBookmarksToTransfer!=0){
	  		mPrefs.edit().remove("numbookmarkedpages").apply();
		...
		if (numSavedTabs>0){
			...
			savedInstancePreferences.edit().clear().apply();
	...
	public void browserActionClicked(View v){
		...
		case R.id.browser_set_home:
			mPrefs.edit().putString("browserhome", WV.getUrl()).apply();
		...
		case R.id.browser_toggle_desktop:
			CheckedTextView cView = (CheckedTextView) v.findViewById(R.id.browser_toggle_desktop);
			if (cView.isSelected());

		    mGlobalPrefs.edit().putBoolean("usedesktopview", !Properties.webpageProp.useDesktopView).apply();
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (Objects.equals(intent.getAction(), Intent.ACTION_WEB_SEARCH) || Objects.equals(intent.getAction(), Intent.ACTION_VIEW)){
        		if (intent.getDataString()!=null){
...
//			ws.setSavePassword(false); // Not needed for API level 18 or greater (deprecat
	...
	@Override   
	 ...
	 if (Properties.webpageProp.closetabsonexit && isFinishing()){
		 savedInstancePreferences.edit().putInt("numtabs", 0).apply();
	 }
	 else{
		   CustomWebView WV = (CustomWebView) webLayout.findViewById(R.id.browser_page);
		   int tabNumber = getTabNumber();
		   savedInstancePreferences.edit().putInt("numtabs", webWindows.size()).apply();
		   
		   if (WV!=null)
			   for (int I=0;I<webWindows.size();I++){
				   savedInstancePreferences.edit().putString("URL"+I, webWindows.get(I).getUrl()).apply();
			   }
		   if (tabNumber==-1)
			   tabNumber = 0;
		   savedInstancePreferences.edit().putInt("tabNumber", tabNumber).apply();
	 }
 }
		...
		if (ActivityCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
			...
			if (ActivityCompat.shouldShowRequestPermissionRationale(this,
				...
				ActivityCompat.requestPermissions(
						this, new String[]{permission.WRITE_EXTERNAL_STORAGE},
